...
program
    ...
    try {
        if (opts.baselineUpdate || opts.updateBaselines) {
            const updated = await updateBaselineSnapshots(opts.url, config.defaultSelectors, width, config.snapshotsDir, 'baseline', browsers);
            console.log(formatBaselineUpdateDone(updated));
            return;
        }
        const outPaths = await Promise.all(browsers.map((b) => captureSnapshot(opts.url, opts.name, opts.selector, width, b)));
...
program
    ...
    .option('--selectors <list>', 'Comma-separated CSS selectors to compare', 'h1,h2,h3,p,a,span')
    .option('--browser <browser>', 'Browser(s) to use: chromium|firefox|webkit|all (comma-separated)', 'chromium')
    ...
    if (isMultiBrowser) {
        const multiResults = diffSnapshotsAllBrowsers(opts.baseline, opts.compare, selectors, thresholdPct, browsers);
        if (opts.json) {
        ...
        const failed = multiResults.some((r) => browsers.some((b) => !r.browsers[b]?.missing && (r.browsers[b]?.diffPercent ?? 0) > thresholdPct));
